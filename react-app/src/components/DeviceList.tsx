...
function DeviceList(props: Props) {
    ...
    const returnPath = "/Devices";

    return <div className="list-group w-75">

        {props.items.map(device =>
            <div key={device._id} className="list-group-item list-group-item-action mb-2 border-top bg-light" aria-current="true">
                <div className="d-flex flex-row">
                    <Link to={"/Devices/" + device.name} state={{ device }} className="d-flex align-items-center w-25 border-end p-2">
                        <h5 className="mb-1">{device.name}</h5>
                    </Link>
                    <Link to={"/Devices/" + device.name} state={{ device }} className="d-flex flex-column align-content-center border-end p-2 text-decoration-none text-reset" style={{ width: '65%' }}>
                        <small className="mb-1">{device.description}</small>
                        <span>{t('device_ip')}: {device.ip}</span>
                    </Link>
