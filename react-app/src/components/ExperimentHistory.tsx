...
function ExperimentHistory(props: Props) {
        ...
        {props.items.map(experiment =>
            <div key={experiment._id} className="list-group-item list-group-item-action mb-2 border-top bg-light" aria-current="true">
                <div className="d-flex flex-row align-items-center">
                    <Link to={"/Devices/" + device.name + "/Experiments/" + experiment.name} state={{ device, experiment }} className="d-flex justify-content-start w-25 border-end">
                        <h6 className="mb-1 text-start">{experiment.name}</h6>
                    </Link>
                    <Link to={"/Devices/" + device.name + "/Experiments/" + experiment.name} state={{ device, experiment }} className="d-flex border-end p-2 text-decoration-none text-reset" style={{ width: '52%' }}>
                        <small className="mb-1">{experiment.description}</small>
                    </Link>
                    <Link to={"/Devices/" + device.name + "/Experiments/" + experiment.name} state={{ device, experiment }} className="d-flex border-end p-2 text-decoration-none text-reset text-end" style={{ width: '13%' }}>
                    ...
                    <div className="d-flex flex-row justify-content-center align-items-center" style={{ width: '7rem' }}>
                        <Link to={"/Devices/" + device.name + "/Experiments/" + experiment.name + "/Edit"} state={{ returnPath, device, experiment }} className="m-auto p-2" aria-label={t('edit')}>
