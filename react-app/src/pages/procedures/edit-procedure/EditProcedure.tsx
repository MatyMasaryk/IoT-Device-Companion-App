...
function EditProcedure() {
    ...
    const resource = '/procedures/' + procedure._id;
    const serverApi = serverUrl + resource;
    const [instructions, setInstructions] = useState(JSON.parse(procedure.instructions));
    ...
    return <div className="text-center d-flex flex-column align-items-center justify-content-center">
        ...
        <form onSubmit={update} className="d-flex flex-column align-items-center w-100">
            <div className="form-group mt-4 w-25">
                <label htmlFor="deviceNameInput">{t('name')}*</label>
                <input defaultValue={procedure.name} type="text" className="form-control" name="name" id="deviceNameInput" placeholder={t('tooltip_name')} required maxLength={64} />
            </div>
            <div className="form-group mt-3 w-25">
                <label htmlFor="deviceDescriptionInput">{t('description')}</label>
                <input defaultValue={procedure.description} type="text" className="form-control" name="description" id="deviceDescriptionInput" placeholder={t('tooltip_description')} maxLength={128} />
            </div>
            ...
            <p className="mt-3">
                <small id="message" className="text-danger"></small>
            </p>
            <button type="submit" className="btn btn-primary mt-3 mb-3">{t('submit')}</button>
        </form>
    </div>
}
